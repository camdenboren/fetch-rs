...
fn main() -> Result<(), anyhow::Error> {
    ...
    let git_path = PathBuf::from(git_path.unwrap());
            ...
            match output_status.into_raw() {
                0 => {
                    let output_stdout = String::from_utf8(output.stdout)
                        .expect("Unable to stringify rebuild_output");
                    tracing::info!("Rebuild succeeded: {}", output_stdout);
                }
                _ => {
                    if cfg.notify {
                        notify(cfg);
                    ...
                    let output_stderr = String::from_utf8(output.stderr)
                        .expect("Unable to stringify rebuild_output");
                    tracing::info!("Rebuild failed: {}", output_stderr);
